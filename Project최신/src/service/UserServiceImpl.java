...
//		====================================================================================
		...
		ArrayList<ArrayList<String>> printReserve = null;
		
		int count = 0;
		
		// 사용자 아이디가 현재 로그인한 아이디인 객체 추출
		ArrayList<ReserveVO> userReserve = null;
		UserVO loginUser = Session.loginUser;
		for(int i = 0; i < reserveDB.size(); i++){
			if(reserveDB.get(i).getUserID() == loginUser.getId()){
		...
		if(userReserve != null){
					...
					if(userReserve.get(i).getShowingCode() == showingDB.get(j).getShowingCode()){
						for(int k = 0; k < movieDB.size(); k++){
							if(showingDB.get(j).getMovieCode() == movieDB.get(k).getMovieCode()){
								printReserve.get(count++).add(movieDB.get(k).getMovieName());
							}
						}
					}
				}
			...
			for(int i = 0; i < userReserve.size(); i++){
				for(int j = 0; j < seatDB.size(); j++){
					if(userReserve.get(i).getSeatCode() == seatDB.get(j).getSeatCode()){
						printReserve.get(count).add(Integer.toString(seatDB.get(j).getScreen()));
						printReserve.get(count++).add(seatDB.get(j).getSeatNum());
					}
				}
			}
			...
			for(int i = 0; i < userReserve.size(); i++){
				for(int j = 0; j < showingDB.size(); j++){
					if(userReserve.get(i).getShowingCode() == showingDB.get(j).getShowingCode()){
						printReserve.get(count++).add(showingDB.get(j).getDate());
						printReserve.get(count++).add(showingDB.get(j).getTime());
					}
				}
			}
			count = 0;
			...
			for(int i = 0; i < printReserve.size(); i++){
				System.out.println(count + 1);
				for(int j = 0; j < printReserve.get(i).size(); j++){
					System.out.print(printReserve.get(i).get(j));
				}
				System.out.println();
			}
		} else {
...
//		}
		...
		seatvo.setSeatNum(what);
		System.out.println("입력한 좌석: " + what);
	...
	public void choiceTime(MovieVO mv, String date, int movieCode) {
		...
		choiceSeat(showingCode);
	}

	

	@Override
	public void choiceSeat(int showingCode) {
		//좌석선택
		Scanner s = new Scanner(System.in);
			...
			if(showingCode == reserveDB.get(i).getShowingCode()){
				for(int j = 0; j < seatDB.size(); j++){
					if(reserveDB.get(i).getSeatCode() == seatDB.get(j).getSeatCode()){
						seatStr = seatDB.get(j).getSeatNum();
						System.out.println("디비 좌석 : " + seatStr);
						System.out.println("reserveDB: " + reserveDB.get(i).getSeatCode());
						System.out.println("seatDB : " + seatDB.get(i).getSeatCode());
						seatTemp[count++] = seatStr;
						break;
					}
				}
